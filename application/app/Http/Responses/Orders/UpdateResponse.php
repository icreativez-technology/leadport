...
class UpdateResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (isset($type) && $type == 'update-assigned') {
            ...
            $html = view('pages/order/components/assigned', compact('task', 'assigned'))->render();
            $jsondata['dom_html'][] = array(
                'selector' => "#task-assigned-container",
                'action' => 'replace',
                'value' => $html);
            //update timer section
            $html = view('pages/order/components/timer', compact('task'))->render();
            $jsondata['dom_html'][] = array(
        ...
        if (isset($type) && $type == 'update-priority') {
            //update display text
            $jsondata['dom_html'][] = [
                'selector' => '#card-order-priority-text',
                'action' => 'replace',
        ...
        if (isset($type) && $type == 'update-vivibility') {
            //update display text
            $jsondata['dom_html'][] = [
                'selector' => '#card-order-client-visibility-text',
                'action' => 'replace',
                'value' => $display_text,
            ];
        ...
        if (isset($type) && $type == 'update-recurring') {
            if (request('source') == 'modal') {
                $html = view('pages/order/components/recurring', compact('task'))->render();
                $jsondata['dom_html'][] = [
