...
class OrderRepository {
     ...
     * Inject dependecies
     */
    public function __construct(Order $tasks) {
        $this->tasks = $tasks;
    }
    ...
    public function search($id = '', $data = []) {
        ...
        $tasks->leftJoin('users', 'users.id', '=', 'orders.task_creatorid');
        //$tasks->leftJoin('clients', 'clients.client_id', '=', 'projects.project_clientid');
        $tasks->leftJoin('tasks_status', 'tasks_status.taskstatus_id', '=', 'orders.task_status');

        //join: users reminders - do not do this for cronjobs
        if (auth()->check()) {
            $tasks->leftJoin('reminders', function ($join) {
                $join->on('reminders.reminderresource_id', '=', 'orders.task_id')
        ...
        $tasks->selectRaw('(SELECT COUNT(*)
                                      ...
                                      AND events.event_parent_id = orders.task_id
        ...
        $tasks->selectRaw('(SELECT COALESCE(SUM(timer_time), 0)
                                           FROM timers WHERE timer_taskid = orders.task_id)
                                           AS sum_all_time');
        ...
        $tasks->selectRaw('(SELECT COUNT(*)
                                      ...
                                      WHERE tasksdependency_taskid = orders.task_id
    ...
    public function create($position = '') {
        ...
        if ($task->save()) {
            ...
            Log::error("record could not be saved - database error", ['process' => '[OrderRepository]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
    }
    ...
    public function timerStop($id) {
        ...
        if ($item->save()) {
            ...
            Log::error("record could not be updated - database error", ['process' => '[OrderRepository]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
    }
