...
use App\Http\Requests\Orders\TaskStoreUpdate;
use App\Http\Responses\Orders\ActivateResponse;
use App\Http\Responses\Orders\ArchiveResponse;
use App\Http\Responses\Orders\AttachFilesResponse;
use App\Http\Responses\Orders\ChecklistResponse;
use App\Http\Responses\Orders\CloneResponse;
use App\Http\Responses\Orders\CloneStoreResponse;
use App\Http\Responses\Orders\contentResponse;
use App\Http\Responses\Orders\CreateResponse;
use App\Http\Responses\Orders\DeleteTaskDependencyResponse;
use App\Http\Responses\Orders\DestroyResponse;
use App\Http\Responses\Orders\IndexKanbanResponse;
use App\Http\Responses\Orders\IndexListResponse;
use App\Http\Responses\Orders\RecurringSettingsResponse;
use App\Http\Responses\Orders\ShowResponse;
use App\Http\Responses\Orders\StoreChecklistResponse;
use App\Http\Responses\Orders\StoreCommentResponse;
use App\Http\Responses\Orders\StoreResponse;
use App\Http\Responses\Orders\StoreTaskDependencyResponse;
use App\Http\Responses\Orders\TimerStartResponse;
use App\Http\Responses\Orders\TimerStopResponse;
use App\Http\Responses\Orders\UpdateChecklistResponse;
use App\Http\Responses\Orders\UpdateErrorResponse;
use App\Http\Responses\Orders\UpdateResponse;
use App\Http\Responses\Orders\UpdateStatusLockedResponse;
use App\Http\Responses\Orders\UpdateStatusResponse;
use App\Http\Responses\Orders\UpdateTagsResponse;
use App\Http\Responses\Orders\UpdateLockedResponse;
...
class Orders  extends Controller {
    ...
    public function __construct(
        ...
        //route middleware
        $this->middleware('ordersMiddlewareTimer')->only([
            'timerStart',
            'timerStop',
            'timerStopAll',
        ]);
        ...
        $this->middleware('ordersMiddlewareParticipate')->only([
            'storeComment',
            'attachFiles',
            'updateStatus',
            'updatePriority',
        ...
        $this->middleware('ordersMiddlewareDeleteAttachment')->only([
            'deleteAttachment',
        ]);

        $this->middleware('ordersMiddlewareDownloadAttachment')->only([
            'downloadAttachment',
        ]);

        $this->middleware('ordersMiddlewareDeleteComment')->only([
            'deleteComment',
        ]);

        $this->middleware('ordersMiddlewareEditDeleteChecklist')->only([
            'updateChecklist',
            'deleteChecklist',
        ...
        $this->middleware('ordersMiddlewareAssign')->only([
            'updateAssigned',
        ]);

        $this->middleware('ordersMiddlewareCloning')->only([
            'cloneTask',
            'cloneStore',
        ]);

        $this->middleware('ordersMiddlewareManageDependencies')->only([
            'storeDependency',
            'deleteDependency',
        ]);
    }
    ...
    public function show(
        ...
        $project_tasks = \App\Models\Order::Where('task_projectid', $task->task_projectid)->orderBy('task_title', 'ASC')->get();
    ...
    public function sendToDocport(Request $request,$id)
    {
        ...
        $createOrder   = $this->orderRequest($request->all());
        ...
        if($response->getStatusCode()  == 201){ 
            $updateTaskstatus  = Order::where('task_id',$id)->update(['send_to_docport'=>1]);
    ...
    public function orderRequest($data)
    {   
        //dd($data['task_custom_field_1']);
        $orderRequestData = array(
            'title'             => $data['title'], 
            'order_status'      => $data['task_status'],
            'order_clientid'    => $data['task_clientid'],
            'order_projectid'   => $data['task_projectid'],
            'order_creatorid'   => $data['task_creatorid'],
            'order_active_state'=> $data['task_active_state'],
            'order_visibility'  => $data['task_visibility'],   
            'transport_type'    => $data['task_custom_field_1'],
            'equipment'         => $data['task_custom_field_2'],  
            'load_type'         => $data['task_custom_field_3'],  
            'quantity'          => $data['task_custom_field_4'],  
            'pickup_index'      => $data['task_custom_field_5'],  
            'pickup_county'     => $data['task_custom_field_6'],  
            'pickup_city'       => $data['task_custom_field_7'],  
            'pickup_address'    => $data['task_custom_field_8'],  
            'pickup_date'       => $data['task_custom_field_9'],  
            'delivery_index'    => $data['task_custom_field_10'],  
            'delivery_county'   => $data['task_custom_field_11'],  
            'delivery_city'     => $data['task_custom_field_12'],  
            'delivery_address'  => $data['task_custom_field_13'],  
            'delivery_date'     => $data['task_custom_field_14'],  
            'incoterms_location'=> $data['task_custom_field_15'],  
            'un_code'           => $data['task_custom_field_16'],  
            'temperature_range' => $data['task_custom_field_17'],  
            'adr_carriage'      => $data['task_custom_field_18'],  
            'fargile_carriage'  => $data['task_custom_field_19'],  
            'remarks'           => $data['task_custom_field_20'],  
            'transport_price'   => $data['task_custom_field_21'],  
            'transit_time'      => $data['task_custom_field_22'],  
        );
        
        $orderStore  = Order::updateOrCreate($orderRequestData);
        return $orderStore;
    }
    ...
    public function updateStatus(ProjectPermissions $projectpermissions, $id) {
        ...
        if ($old_status != request('task_status')) {
            if ($first_task = \App\Models\Order::Where('task_status', request('task_status'))->orderBy('task_position', 'ASC')->first()) {
    ...
    public function updateVisibility($id) {
        ...
        $payload = [
            ...
            'tasks' => $tasks,
            'stats' => $this->statsWidget(),
            'display_text' => ($task->task_lclient_visibiity == 'yes') ? __('lang.visible') : __('lang.hidden'),
        ];
    ...
    public function updateClient($id , Request $request){
        ...
        $taskClient = Order::where('task_id',$id)->update(['client'=>$client]);
        return $taskClient;
    }
    ...
    public function toggleStatus() {
        ...
        //get the task
        $tasks = $this->taskrepo->search(request()->route('order'));
        $task = $tasks->first();
    ...
    private function applyPermissions($task = '') {
        //sanity - make sure this is a valid task object
        if ($task instanceof \App\Models\Order) {
            //[sql optimisation]
            ...
            if(isset($project) && $project != null){
                ...
                if (isset($task->task_projectid) && count($assigned_users) && count($assigned_project_users) && count($project_managers)) {
                    //edit permissions
            ...
            if ($task->task_projectid)  {
                //edit permissions
                $task->permission_edit_task = (auth()->user()->role->role_templates_projects >= 1) ? true : false;
                //delete permissions
                $task->permission_delete_task = (auth()->user()->role->role_templates_projects >= 1) ? true : false;
                //delete participate
                $task->permission_participate = (auth()->user()->role->role_templates_projects >= 1) ? true : false;
                //super user
                $task->permission_assign_users = true;
                //super user
    ...
    public function archive($id) {
        ...
        $task = \App\Models\Order::Where('task_id', $id)->first();
    ...
    public function updateCustomFields($id, Request $request) {
       ...
       //dd($request->goods);
        ...
        $request->request->remove('goods');
        $request->request->remove('visibility_left_menu_toggle_button');
        $request->request->remove('system_language');
        $request->request->remove('system_languages');
        $request->request->remove('user_has_due_reminder');
        $request->request->remove('projects_menu_list');
        $request->request->remove('resource_query');
        $request->request->remove('filter_show_archived_tasks');
        
        $task = Order::where('task_id',$id)->update($request->all());
    ...
    public function stopRecurring() {
        ...
        //get the task
        $task = \App\Models\Order::Where('task_id', request()->route('order'))->first();
    ...
    public function storeDependency(TaskDependencyRepository $dependencyrepo, $id) {
        ...
        if (!$blocking_task = \App\Models\Order::Where('task_id', request('tasksdependency_blockerid'))->first()) {
    ...
    private function pageSettings($section = '', $data = []) {
        ...
        $page = [
            ...
            'dynamic_search_url' => url('orders/search?action=search&taskresource_id=' . request('taskresource_id') . '&taskresource_type=' . request('taskresource_type')),
            'add_button_classes' => '',
            'heading'=> 'Orders',
        ...
        $page += [
            ...
            'add_modal_create_url' => url('orders/create?taskresource_id=' . request('taskresource_id') . '&taskresource_type=' . request('taskresource_type')),
            'add_modal_action_url' => url('orders?taskresource_id=' . request('taskresource_id') . '&taskresource_type=' . request('taskresource_type') . '&count=' . ($data['count'] ?? '')),
    ...
    private function statsWidget($data = array()) {
        ...
        foreach (config('task_statuses') as $status) {
            $stat = [
                'value' => \App\Models\Order::where('task_status', $status->taskstatus_id)->count(),
