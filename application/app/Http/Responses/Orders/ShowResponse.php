...
class ShowResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        // RIGHT PANEL---
        $html = view('pages/order/rightpanel', compact(
            'page',
            'task',
            'assigned',
            'milestones',
        ...
        $jsondata['dom_browser_url'] = [
            'title' => __('lang.order') . ' - ' . $task->task_title,
            'url' => url("/orders/v/" . $task->task_id . "/" . str_slug($task->task_title)),
        ];

        //show tabs menu
        $html = view('pages/order/content/tabmenu', compact('task'))->render();
        $jsondata['dom_html'][] = [
