...
<div class="col-md-12  col-lg-7 p-b-9 align-self-center text-right {{ $page['list_page_actions_size'] ?? '' }} {{ $page['list_page_container_class'] ?? '' }}"
    ...
    <div id="list-page-actions">
        ...
        <button type="button" data-toggle="tooltip" title="{{ cleanLang(__('lang.show_archive_orders')) }}"
            ...
            data-url="{{ urlResource('/orders/search?action=search&toggle=pref_filter_show_archived_tasks') }}">
        ...
        <button type="button" data-toggle="tooltip" title="{{ cleanLang(__('lang.my_orders')) }}"
            ...
            data-url="{{ urlResource('/orders/search?action=search&toggle=pref_filter_own_tasks') }}">
        ...
        <button type="button" data-toggle="tooltip" title="{{ cleanLang(__('lang.kanban_view')) }}"
            ...
            data-url="{{ urlResource('/orders/search?action=search&toggle=layout') }}">
        ...
        <div class="btn-group" id="list_actions_sort_kanban">
            ...
            <div class="dropdown-menu dropdown-menu-right fx-kaban-sorting-dropdown">
                ...
                <a class="dropdown-item js-ajax-ux-request" id="sort_kanban_task_created" href="javascript:void(0)"
                    data-url="{{ urlResource('/orders?action=sort&orderby=task_created&sortorder=asc') }}">{{ cleanLang(__('lang.date_created')) }}</a>
                <a class="dropdown-item js-ajax-ux-request" id="sort_kanban_task_date_start" href="javascript:void(0)"
                    data-url="{{ urlResource('/orders?action=sort&orderby=task_date_start&sortorder=asc') }}">{{ cleanLang(__('lang.start_date')) }}</a>
                <a class="dropdown-item js-ajax-ux-request" id="sort_kanban_task_date_due" href="javascript:void(0)"
                    data-url="{{ urlResource('/orders?action=sort&orderby=task_date_due&sortorder=asc') }}">{{ cleanLang(__('lang.due_date')) }}</a>
                <a class="dropdown-item js-ajax-ux-request" id="sort_kanban_task_title" href="javascript:void(0)"
                    data-url="{{ urlResource('/orders?action=sort&orderby=task_title&sortorder=asc') }}">{{ cleanLang(__('lang.title')) }}</a>
            </div>
        </div>
        @endif
