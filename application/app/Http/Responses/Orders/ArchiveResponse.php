...
class ArchiveResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('ref') == 'list' || request('ref') == '') {
            ...
            if (auth()->user()->pref_filter_show_archived_orders == 'no') {
            ...
            $jsondata['dom_classes'][] = [
                'selector' => ".card_archive_button_" . $orders->first()->task_id,
                'action' => 'add',
                'value' => 'hidden',
            ];
            $jsondata['dom_classes'][] = [
                'selector' => ".card_restore_button_" . $orders->first()->task_id,
                'action' => 'remove',
                'value' => 'hidden',
            ];
            $jsondata['dom_visibility'][] = [
                'selector' => "#archived_icon_" . $orders->first()->task_id,
                'action' => 'show',
            ];
            $jsondata['dom_visibility'][] = [
                'selector' => "#card_archived_notice_" . $orders->first()->task_id,
                'action' => 'show',
            ];
        ...
        if (request('ref') == 'page') {
            ...
            $jsondata['redirect_url'] = url("orders/" . $orders->first()->task_id);
        }
