...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the orders
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            if (auth()->user()->pref_filter_own_orders == 'yes') {
                $jsondata['dom_classes'][] = [
                    'selector' => '#pref_filter_own_orders',
                    'action' => 'add',
                    'value' => 'active',
                ];
            } else {
            ...
            $template = 'pages/orders/wrapper';
            //standard view
            $page['url'] = loadMoreButtonUrl($tasks->currentPage() + 1, request('source'));
            $page['loading_target'] = 'order-td-container';
