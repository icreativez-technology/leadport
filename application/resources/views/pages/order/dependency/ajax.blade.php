...
<div id="task_dependency_task_{{ $dependency->tasksdependency_id }}"
    ...
    <span><a href="{{ url('/orders/v/'.$dependency->task_id) }}" target="_blank">{{ $dependency->task_title }}</a></span>
    ...
    <span class="task-dependency-delete-button ajax-request" id="task-dependency-delete-button" data-parent="task_dependency_task_{{ $dependency->tasksdependency_id }}"
        data-url="{{ urlResource('/orders/'.$task->task_id.'/delete-dependency?dependency_id='.$dependency->tasksdependency_id) }}" data-ajax-type="DELETE"
