...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [timer] process for the orders
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
    ...
    public function toResponse($request) {
        ...
        if (isset($task_id) && is_numeric($task_id)) {
            ...
            $jsondata['dom_classes'][] = array(
                'selector' => '#order_timer_table_' . $task_id,
                'action' => 'remove',
                'value' => 'timer-running');
            $jsondata['dom_classes'][] = array(
                'selector' => '#order_timer_card_' . $task_id,
                'action' => 'remove',
                'value' => 'timer-running');
            $jsondata['dom_visibility'][] = [
                'selector' => '#card-order-timer-' . $task_id,
                'action' => 'hide',
            ];
